...
export const getLastQuoteId = async (
  ...
  try {
    const lastQuote = await Quote.findOne({ companyId })
      .sort({ CreatedAt: -1 }) // Sort by creation date instead of _id
      .select('quoteId')
      .lean<{ quoteId: string } | null>()

    if (!lastQuote || !lastQuote.quoteId) {
      return 0
    }

    // Extract the numeric part from the formatted quote ID
    const numericPart = lastQuote.quoteId.replace(/[^\d]/g, '')
    const lastQuoteId = parseInt(numericPart, 10)
    logger.info(`Last quote ID for company ${companyId}: ${lastQuoteId}`)
    return isNaN(lastQuoteId) ? 0 : lastQuoteId
  } catch (error) {
    logger.error(
...
export const getCompanyById = async (
  ...
  try {
    const company = await Company.findById(companyId).lean<CompanyType | null>()
    if (!company) {
    ...
    logger.error(`Error fetching company details for ID ${companyId}: ${error}`)
