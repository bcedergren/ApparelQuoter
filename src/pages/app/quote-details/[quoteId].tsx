...
const QuoteDetailsPage = () => {
      ...
      if (quote && quote.companyId) {
        try {
          ...
          if (data?.prices?.screenPrinting) {
            ...
            toast.error('Screen printing prices not found in the API response')
            console.error(
      ...
      if (quote && quote.companyId) {
        try {
          ...
          if (data.customers && data.customers.length > 0) {
            ...
            toast.error('No customers found for this company.')
            setCustomer(null)
          }
        } catch (err: Error | any) {
  ...
  useEffect(() => {
    if (prices && quote && customer && company) {
      ...
      setBalance(totalCost - (totalCost * (quote.depositPercentage || 0)) / 100)
