...
const PlaceNewDesignPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSupportingData = async () => {
      ...
      try {
        ...
        if (customersRes.ok) {
          setCustomers((customersJson.customers || []).map((c: any) => ({ value: c._id, label: c.companyName || c.contactName || c.name || 'Customer' })));
        }
        if (quotesRes.ok) {
  ...
  // Load existing images (versions) for selected customer
  ...
  const onSelectLogo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // keep focus available for re-uploading same file
  ...
  const createAndGo = async () => {
    ...
    try {
      ...
      // If a logo was chosen (uploaded or existing), create a Version tied to this design
      ...
      // Save initial placement if we have both apparel and a logo version
      ...
      setError(e.message || 'Failed to create design');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Container fluid className="mt-3">
        <Row>
          <Col lg={8} className="mb-3">
            <Card>
              <Card.Header>Start Placement</Card.Header>
              <Card.Body>
                {error && (
                ...
                >
                  ...
                  {logoPreviewUrl && (
                    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 8 }} className="d-flex justify-content-center">
                      <div className="bg-white bg-opacity-75 border rounded px-3 py-2" style={{ backdropFilter: 'blur(2px)' }}>
                        <div className="d-flex align-items-center gap-2">
                          <small className="text-muted">Use handles to move/resize. Rotation below.</small>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className="mt-3 d-flex justify-content-end">
                  <Button onClick={createAndGo} disabled={saving || !canSave}>{saving ? 'Creating...' : 'Create & Continue'}</Button>
                </div>
              </Card.Body>
            </Card>
          </Col>
...
export default PlaceNewDesignPage;
